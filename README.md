...
This repository implements Source Sequence acquisition, Candidate K-mer filtering, optional Taxonomic Exact-Match Screening, and verified Deacon Index construction. It enumerates canonical Candidate K-mers, cancels exact Interference Background matches, and removes low-complexity k-mers with Deacon to produce a Locally Filtered Bait Set when survivors remain. When supplied a Taxonomic Reference Database, it screens those Baits with full-length exact BLAST matches under the Target Taxon. Threshold calibration remains for a later commit.
