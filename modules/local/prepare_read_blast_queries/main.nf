process PREPARE_READ_BLAST_QUERIES {
    ...
    input:
    tuple val(meta), path(counts, stageAs: 'counted_reads/*'), path(statuses, stageAs: 'candidate_statuses/*'), path(unique_fasta)
    ...
    tuple val(meta), path('read_blast_search_parameters.tsv'), optional: true, emit: terminal_search_parameters
    ...
    prepare_read_blast_queries.py \
        ...
        --terminal-search-parameters-out read_blast_search_parameters.tsv \
