process RESOLVE_OPTIMIZATION_READS {
    ...
    input:
    tuple val(meta), path(optimization_read_set)

    output:
    tuple val(meta), path('optimization_reads.tsv'), path(optimization_read_set), emit: manifest
    ...
    script:
    """
    resolve_optimization_reads.py \
        --design-id ${meta.id} \
        --directory ${optimization_read_set} \
        --manifest-out optimization_reads.tsv
    """
}
